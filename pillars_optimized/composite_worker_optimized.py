...
def _load_comp_cfg(path: str=DEFAULT_COMP_INI)->dict:
    ...
    tfw_raw = cp.get("composite.tfs","list",fallback="15m:0.1,30m:0.3,60m:0.3,90m:0.2,120m:0.1")
    tfw: Dict[str,float] = {}
...
def _score_per_kind(symbol:str, kind:str, Wtf:Dict[str,float])->Tuple[Dict[str,float], Dict[str,bool]]:
    ...
    out["confidence"] = 50.0 # Placeholder
...
def process_symbol_unified(symbol:str, comp_ini:str=DEFAULT_COMP_INI)->int:
    ...
    print(f"✅ {symbol} -> {final_score:.1f} (Veto={hard_veto}) -> Strategies: {suggested_strategies}")
    return 1
