...
# We can reuse the config from the original strategy for database connections
from ..ml_strategy import config
...
def load_all_tables_for_featuretools(engine):
    ...
    tables["spot_candles"] = pd.read_sql(
        f"SELECT ts, symbol, close, volume, high, low FROM {config.TABLES['spot_candles']} WHERE interval = '15m'",
        engine, parse_dates=["ts"],
    )

    # Intraday futures candles (15m)
    tables["futures_candles"] = pd.read_sql(
        f"SELECT ts, symbol, close as fut_close, oi FROM {config.TABLES['futures_candles']} WHERE interval = '15m'",
        engine, parse_dates=["ts"],
    )
