...
def score_flow(
    ...
    try:
        ...
        if ml_enabled and n >= ml_min and tf == "15m":
            # 2.1 Get raw prob from indicators.ml_pillars
            ml_prob = _load_ml_prob_from_db(symbol, kind, tf, ts, cfg)

            # Path B: optional Python callback if you later want on-the-fly inference
            if ml_prob is None:
