...
def _mom_features(dtf: pd.DataFrame, symbol: str, kind: str, tf: str, cfg: dict, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def get_meta(key):
        if context and key in context: return float(context[key])
        v = last_metric(symbol, kind, tf, key)
    ...
    r1_dist = get_meta("PIVOT.r1.dist_pct")
    near_r1_break = (r1_dist < 0.5 and r1_dist > -0.5)
    ...
    feat: Dict[str, float | int | bool] = {
        ...
        "near_r1_break": bool(near_r1_break)
    }
    return feat
