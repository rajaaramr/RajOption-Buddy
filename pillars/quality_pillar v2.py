...
def _load_quality_scenarios(cfg: dict) -> Tuple[str, List[dict]]:
    cp = cfg["cp"]
    names = []
    if cp.has_section("quality_scenarios"):
        raw = cp.get("quality_scenarios", "list", fallback="")
        raw = raw.replace("\n", " ")
...
# Scenario engine
# -----------------------------
def _run_scenarios(features: Dict[str, Any], cfg: dict, scenario_names: List[str]) -> Tuple[float, bool, bool, Dict[str, float], List[tuple]]:
    cp = cfg["cp"]
    total = 0.0
    veto = False
    reversal = False
    ...
    for name in scenario_names:
        ...
        if when_tree and _safe_eval(when_tree, features):
            ...
            # Flags
            if cp.getboolean(sec, "set_veto", fallback=False): veto = True
            if cp.getboolean(sec, "set_reversal", fallback=False): reversal = True
...
def score_quality(symbol: str, kind: str, tf: str, df5: pd.DataFrame, base: BaseCfg, 
    ...
    cfg = _cfg(ini_path)
    rules_mode, scenario_names = _load_quality_scenarios(cfg)

    # Build Features (with Context)
    feats = _build_features(dftf, symbol, kind, tf, cfg, context)

    # Run Engine
    total, veto, reversal, parts, fired = _run_scenarios(feats, cfg, scenario_names)
