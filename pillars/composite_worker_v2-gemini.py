# scheduler/composite_worker_v2.py
...
# Optimization: Import SCORER, not just the runner
from pillars.structure_pillar import score_structure 
...
def _load_comp_cfg(path: str=DEFAULT_COMP_INI)->dict:
    ...
    # Bias for Futures vs Spot
    merge_bias_fut = cp.getfloat("composite","merge_bias_fut",fallback=0.6) 
...
def _fetch_context(symbol: str, kind: str) -> Dict[str, Any]:
    ...
    # List of columns to pre-fetch for context
    cols = [
        "pivot_p", "pivot_r1", "pivot_s1", "pivot_r2", "pivot_s2",
        "vp_poc", "vp_vah", "vp_val", "bb_score"
    ]
    
    # Check which columns actually exist in DB to avoid crashes
    with get_db_connection() as conn, conn.cursor() as cur:
        cur.execute("""
    ...
    # Get latest row (simplified context fetch)
    with get_db_connection() as conn, conn.cursor() as cur:
        cur.execute(f"""
    ...
    ctx = {}
    col_list = list(existing) # Ordering matters for row indexing
    # Re-query to match ordering if set doesn't preserve it? 
    # Safer:
    for i, col in enumerate(col_list):
        # Map DB columns to Context Keys expected by Pillars
        val = row[i]
        if val is not None:
            ...
            else: ctx[col] = float(val) # Pivots map directly by name usually
...
def _score_per_kind(symbol:str, kind:str, Wtf:Dict[str,float])->Tuple[Dict[str,float], Dict[str,bool]]:
    ...
    out: Dict[str,float] = {"trend":None,"momentum":None,"quality":None,"flow":None,"structure":None,"risk":None}
    veto: Dict[str,bool] = {"quality":False,"flow":False,"structure":False,"risk":False}

    if dfs.get("15m", pd.DataFrame()).empty:
        return out, veto

    # 1. Fetch Context (Pivots, VP) ONCE
    context = _fetch_context(symbol, kind)

    per_tf = {"TREND":{}, "MOMENTUM":{}, "QUALITY":{}, "FLOW":{}, "RISK":{}, "STRUCT":{}}
    veto_tf = {"QUALITY":{}, "FLOW":{}, "RISK":{}, "STRUCT":{}}

    # 2. Loop Timeframes (In Memory)
    ...
    for tf in TF_SET:
        ...
        # --- Structure (In-Memory, no DB write-read loop) ---
        try:
            r = score_structure(symbol, kind, tf, dftf, base_cfg, context=context)
            if r:
                per_tf["STRUCT"][tf] = float(r[1])
                veto_tf["STRUCT"][tf] = bool(r[2]) # Structure has vetos too
        except Exception: pass
...
def _merge_spot_fut(spot, fut, v_spot, v_fut, bias):
    ...
    merged = {}
    for k in ["trend", "momentum", "quality", "risk", "structure"]:
    ...
    # Flow is special (often futures only)
...
def process_symbol_unified(symbol:str, comp_ini:str=DEFAULT_COMP_INI)->int:
    ...
    # 4. Final Formula
    base_score = sum(w[k] * nz(merged[k]) for k in w if k in merged)
    ...
    rows = [(
        ...
        nz(merged.get("trend")), nz(merged.get("momentum")), nz(merged.get("quality")),
        nz(merged.get("flow")),  nz(merged.get("structure")), nz(merged.get("risk")),
...
def run(symbols: Optional[List[str]]=None):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        # We use a helper wrapper to catch exceptions per process
